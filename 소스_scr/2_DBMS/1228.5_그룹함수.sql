--[V]그룹함수 SUM,AVG,MIN,MAX,COUNT,STDDEV표준펀차,VARIANCE분사
SELECT ENAME,ROUND(SAL,-3)FROM EMP;--단일행함수
SELECT SUM(SAL)FROM EMP;--그룹함수
SELECT ENAME, SUM (SAL) FROM EMP;--그룹함수는 단일행과 같이 쓸 수 없음! 
SELECT DEPTNO, SUM(SAL) FROM EMP GROUP BY DEPTNO;--단일행과 그룹함수와 같이 쓸떄는 무조건 GROUP BY는 필수이다! 
--EMP테이블에서 평균 SAL
SELECT ROUND(AVG(SAL),2) FROM EMP;
SELECT COUNT(*) FROM EMP;--COUNT 행수! --로우(행)수!
SELECT TRUNC(AVG(SAL)), TRUNC(SUM(SAL)/COUNT(SAL)) FROM EMP;
SELECT AVG(COMM) FROM EMP; -- 그룹함수는 NULL값을 제외! 
SELECT COUNT(COMM),SUM(COMM),AVG(COMM) FROM EMP; -- NULL값은 모두 제외하고 남은 갯수 4개! 
...
SELECT DEPTNO 부서번호,MAX(SAL)   
    FROM EMP 
    GROUP BY DEPTNO
    ORDER BY 부서번호;--GROUP BY절에는 필드의 별칭이 올 수 없다! 
...
SELECT DEPTNO, MIN(HIREDATE),MAX(HIREDATE) FROM EMP
    GROUP BY DEPTNO;
SELECT MIN(ENAME),MAX(ENAME) FROM EMP;--A에 가장 가까운 단어가 MIN Z에 가장 가까운 단어가 MAX 
SELECT COUNT(JOB) FROM EMP;--JOB필드의 갯수! 
SELECT COUNT(DISTINCT JOB) FROM EMP;

--	가장 최근에 입사한 사원의 입사일과 입사한지 가장 오래된 사원의 입사일을 출력. 
--(결과) 80/12/17:14620일째 83/01/12:13864일째
SELECT MIN(HIREDATE)||'최초입사:'||TRUNC (SYSDATE-MIN(HIREDATE))||'일쨰'||
        MAX(HIREDATE)||'최근입사:'||TRUNC(SYSDATE-MAX(HIREDATE))||'일째' FROM EMP;	
        
--(결과) 80년12월17일 최초입사  :14,620일째 83년01월12일 최근입사 :13,864일째?	
SELECT MIN(HIREDATE)||'최초입사:'||TO_CHAR(TRUNC (SYSDATE-MIN(HIREDATE)),'99,999')||'일쨰'||
...
SELECT DNAME,COUNT(*),MAX(SAL),MIN(SAL),ROUND(AVG(SAL))
    ...
    GROUP BY DNAME;--그룹함수이름과 앞에 이름이 꼭 같아야함
...
SELECT DEPTNO, ROUND(AVG(SAL)) FROM EMP   
        GROUP BY DEPTNO
        HAVING AVG(SAL)>2000;--WHERE절에는 그룹함수를 집어 넣으면 안되용 
...
SELECT DEPTNO 부서별, ROUND(AVG(SAL))급여평균, SUM(SAL)급여합계, MIN(SAL) 최소급여
    FROM EMP
    GROUP BY DEPTNO;

--12. 위의 11번을 수정하여, 부서별 급여평균 최대치, 부서별 급여합의 최대치, 
            --부서별 최소급여의 최대치를 출력하라.
SELECT MAX(ROUND(AVG(SAL)))급여평균, MAX(SUM(SAL))급여합계, MAX(MIN(SAL)) 최소급여
    FROM EMP
    GROUP BY DEPTNO;
...
--  14		  1	     10	      2	      1
SELECT SUBSTR(TO_CHAR(HIREDATE,'YYYY-MM-DD'),1,4)년수, COUNT(*) TOTAL  FROM EMP
